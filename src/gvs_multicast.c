...
int df_gvs_multicast_prepare(struct df_gvs_multicast *membership,
    ...
    if (membership == NULL || identity == NULL || local_address == NULL ||
        inet_pton(AF_INET, local_address, &parsed) != 1) return DF_ERR_INVALID;
    memset(membership, 0, sizeof(*membership));
    membership->fd = -1;
    membership->port = DF_GVS_MULTICAST_PORT;
    if (df_gvs_identity_multicast_ip(identity, membership->group) != DF_OK ||

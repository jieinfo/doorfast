...
assert.equal(byName('gvs_interface', passive).label, '被动观察接口');
assert.equal(byName('gvs_interface', passive).dependency.value, '0');
...
assert.equal(byName('gvs_interface', host).label, '主机模式接口');
assert.equal(byName('gvs_interface', host).dependency.value, '1');

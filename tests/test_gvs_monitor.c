...
void test_gvs_monitor_persistent_preview_keeps_retrying(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x82,
        NULL, 0U, 0x01020304U, now_ms + 1U, &result));
    TEST_ASSERT_INT_EQ(1, result.retry_ready ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
    TEST_ASSERT_INT_EQ(DF_OK,
        df_gvs_monitor_step(&monitor, now_ms + 1U, &action));
    TEST_ASSERT_INT_EQ(1, action.send ? 1 : 0);
    TEST_ASSERT_INT_EQ(0x04, action.opcode);
...
void test_gvs_monitor_persistent_preview_retries_after_first_frame_timeout(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_step(&monitor,
        101U + DF_GVS_MONITOR_FIRST_FRAME_TIMEOUT_MS, &action));
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_STOPPING, monitor.state);
    TEST_ASSERT_INT_EQ(0, monitor.failure);
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_step(&monitor,
        101U + DF_GVS_MONITOR_FIRST_FRAME_TIMEOUT_MS, &action));
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x82,
        NULL, 0U, 0x01020304U,
        101U + DF_GVS_MONITOR_FIRST_FRAME_TIMEOUT_MS + 1U, &result));
    TEST_ASSERT_INT_EQ(1, result.retry_ready ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_step(&monitor,
        101U + DF_GVS_MONITOR_FIRST_FRAME_TIMEOUT_MS + 1U, &action));
    TEST_ASSERT_INT_EQ(1, action.send ? 1 : 0);
    TEST_ASSERT_INT_EQ(0x04, action.opcode);
}
...
void test_gvs_monitor_busy_response_retries_beyond_three_requests(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_STOPPING, monitor.state);
    TEST_ASSERT_INT_EQ(1, monitor.retry_after_stop ? 1 : 0);
}
...
void test_gvs_monitor_busy_cleanup_counts_rejections_not_requests(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x50U,
        NULL, 0U, 0x01020304U, now_ms, &result));
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
...
void test_gvs_monitor_retries_after_station_ends_short_preview(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x02U,
        preview_end, sizeof(preview_end), 0x01020304U, 200U, &result));
    TEST_ASSERT_INT_EQ(1, result.retrying ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
    TEST_ASSERT_INT_EQ((int)generation, (int)monitor.generation);
    TEST_ASSERT_INT_EQ(1, (int)monitor.request_attempts);
    TEST_ASSERT_INT_EQ(0, monitor.media_ready ? 1 : 0);
    retry_deadline = monitor.next_action_ms;
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x02U,
        preview_end, sizeof(preview_end), 0x01020304U, 300U, &result));
    TEST_ASSERT_INT_EQ(1, result.retrying ? 1 : 0);
    TEST_ASSERT_INT_EQ((int)retry_deadline, (int)monitor.next_action_ms);
    TEST_ASSERT_INT_EQ(DF_ERR_INVALID, df_gvs_monitor_admit_jpeg(&monitor,
    ...
    TEST_ASSERT_INT_EQ(0x04, action.opcode);
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x02U,
        preview_end, sizeof(preview_end), 0x01020304U, 1201U, &result));
    TEST_ASSERT_INT_EQ(1, result.retrying ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
...
void test_gvs_monitor_end_notification_retries_without_accepting_stale_video(void) {
    ...
    TEST_ASSERT_INT_EQ(1, result.keepalive_reply ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
    TEST_ASSERT_INT_EQ(1, monitor.retry_waiting ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_ERR_INVALID, df_gvs_monitor_admit_jpeg(&monitor,
        station, local, 0x01020304U, 7U, 301U, &result));
...
void test_gvs_monitor_persistent_preview_resumes_after_peer_end(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x02U,
        peer_end, sizeof(peer_end), 0x01020304U, 200U, &result));
...
void test_gvs_monitor_preview_retry_waits_for_stop_ack(void) {
    ...
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x84U,
        confirmation, sizeof(confirmation), 0x01020304U, 101U, &result));
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_admit_jpeg(&monitor, station,
        local, 0x01020304U, first_generation, 102U, &result));
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_mark_publishing(&monitor,
        first_generation, 103U));
    ...
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_STOPPING, monitor.state);
    TEST_ASSERT_INT_EQ(DF_ERR_INVALID, df_gvs_monitor_admit_jpeg(&monitor,
        station, local, 0x01020304U, first_generation, 201U, &result));
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_ADMIT_REJECT_STATE,
        result.admit_reject);
    TEST_ASSERT_INT_EQ(DF_OK, monitor_receive(&monitor, station, local, 0x82U,
        NULL, 0U, 0x01020304U, 300U, &result));
    TEST_ASSERT_INT_EQ(1, result.retry_ready ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
    TEST_ASSERT_INT_EQ(1, monitor.generation > first_generation ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_step(&monitor, 300U, &action));
    TEST_ASSERT_INT_EQ(1, action.send ? 1 : 0);
    TEST_ASSERT_INT_EQ(0x04, action.opcode);
}
...
void test_gvs_monitor_preview_retry_falls_back_after_stop_timeout(void) {
    ...
    TEST_ASSERT_INT_EQ(1, action.send ? 1 : 0);
    TEST_ASSERT_INT_EQ(DF_OK, df_gvs_monitor_step(&monitor,
        200U + DF_GVS_MONITOR_STOP_TIMEOUT_MS, &action));
    TEST_ASSERT_INT_EQ(DF_GVS_MONITOR_REQUESTING, monitor.state);
    TEST_ASSERT_INT_EQ(1, monitor.retry_ready ? 1 : 0);

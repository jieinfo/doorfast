...
运行时使用独立查询调度器，每 1000 ms 最多发出一次 `08/03`，不与召梯事务共享
完成条件。查询失败只计数并等待下一周期，避免因未知设备阻塞主循环。
...
- 为 `gvs_udp_sender` 增加电梯发送适配，复用观察到的对端 IP 路由；（已实现）
- 仅在 `active_host` 模式开放 `call_elevator`，参数为方向，不接受任意 payload；（已实现）
- 状态公开事务状态、方向、尝试次数、原始状态值和数据年龄，不公开完整私有帧；（已实现）
- 被动模式、无效身份、旧事务、重复请求和未配置电梯能力均拒绝发送；自动召梯只在
  显式启用、主机模式时触发，来电固定使用上行方向，每个新会话代次最多一次；配置
  方向字段保留给后续手动策略，不改变来电自动上行规则；
- 使用厂商 PCAP 黄金样本、UDP 回环和 ImmortalWrt APK 验证后再合并。（本 PR 已完成 PCAP/UDP/主机验证，APK 待 Actions）

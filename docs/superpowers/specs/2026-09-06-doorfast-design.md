...
The first release supports passive monitoring, notifications, automatic door
opening, automatic hangup, and elevator actions. It operates only on network
interfaces explicitly selected by the administrator or discovered from local
...
`capture_promiscuous`. The `auth`, `auto_update`, remote deployment, and
vendor-cloud-only settings are intentionally absent.
...
templated body. TLS certificate validation remains enabled. Both integration
targets use a bounded queue; unreachable targets are retried a finite number
of times and cannot block local door-session handling.

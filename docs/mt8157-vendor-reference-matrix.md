...
| 召梯 | `015-elevator.md`、`docs/doorfast-disconnect-20260911.pcap` | `08/02` 是 42 字节公共头加 4 字节 payload，总长 46 字节；上/下方向；立即发送、1000 ms 后最多重试一次、2000 ms 硬截止；`08/82` 只结束协议事务 | 46 字节构帧和纯单槽控制器已通过 `D-T`；请求、协议结果、超时和实体结果分开报告；尚未接入守护进程 UDP | 尚无 `08/82` 现场样本和 `D-F`，不能宣称召梯成功 |
| 电梯状态 | `015-elevator.md`、`docs/doorfast-disconnect-20260911.pcap` | `08/03` 是 42 字节零 payload 查询；`08/83` 返回数量及楼层/状态二元组 | 42 字节构帧及最多 8 项的 `1 + 2 * count` 有界解析已通过 `D-T`，保留原始值和扩展长度；尚未接入守护进程 UDP | 尚无 `08/83` 现场样本和 `D-F`；`08/01` 语义未闭合 |

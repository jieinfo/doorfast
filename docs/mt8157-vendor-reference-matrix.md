...
“厂商交付”描述材料来源，不自动表示每份文件都是厂商正式发布的协议规范。
例如数据集内的 `gvs-compatible-host-development-manual.md` 明确把自身定义为兼容性
...
| 门禁控制 | `009-entrance-guard-unlock.md`、`029-gvs-entrance-guard-payload-result.md` | 直接路径 `04/09 -> 04/89`；挑战路径 `04/11 -> 04/91`；直接帧保留“声明 12、实际 body 8”兼容形状 | 独立 access adapter；安装密钥不入日志；只有终端回复可形成协议成功/失败 | 失败码全集、挑战异常和物理门锁动作需 `D-F` |
| 召梯 | `015-elevator.md`、相关 PCAP | `08/02`，4 字节 payload；上/下方向；立即发送并按 1 秒间隔有限重试；`08/82` 结束事务 | 独立 elevator adapter；去重点击；请求、协议结果、超时分开报告 | 尚无 `08/82` 现场样本，不能宣称召梯成功 |
| 电梯状态 | `015-elevator.md` | `08/03` 零 payload 查询；`08/83` 返回数量及楼层/状态二元组 | 有界解析 `1 + 2 * count`，保留原始楼层和状态值 | 尚无 `08/83` 现场样本；`08/01` 语义未闭合 |
...
按静态调用链解码为：`direction=0x00`（下行入口），其余三个位置为
`BCD(g[3])`、`g[3]`、`g[4]`。这里保留位置含义，不把匿名化值解释成用户楼层。

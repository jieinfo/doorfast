...
The `r9` target run upgraded the installed `r8` package without changing its UCI identity or persisted synchronization version. It observed two `peer_reply_frame prepared=1 length=48 attempt=1 header=placeholder mode=memory` records and two `sending`/`success` pairs with `attempt=1 timed_out=0 mode=simulated`. This proves that the new complete-context header-provider contract works in the target x86_64 build and that each pending FIFO item still reaches the simulated terminal state. It does not prove that the placeholder-header frame was transmitted or accepted by a door station.

...
The `r7` runner adds an offline-send acceptance boundary. Each fixed `07/01` must be followed by `peer_reply_tx state=success attempt=1 timed_out=0 mode=simulated`. This proves that the target APK drains the pending FIFO through the transaction state machine; it does not prove that a `07/81` frame was serialized, transmitted, or accepted by a door station.

...
This verifies QEMU forwarding, Ethernet capture, UDP extraction, complete common-header parsing, candidate online refresh and timeout, runtime election integration, and call-event dispatch in the built x86_64 APK. Capture buffering requires bounded polling rather than a fixed short sleep. A reply received after election can be parsed successfully without changing the role.
